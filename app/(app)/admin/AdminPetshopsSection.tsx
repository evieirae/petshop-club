...
import { atualizarTaxasPlataforma } from "./actions";
...
export function AdminPetshopsSection({ petshops }: { petshops: Petshop[] }) {
  if (petshops.length === 0) {
    return (
      <EmptyState
        titulo="Nenhum petshop cadastrado"
        descricao="Petshops novos entram direto pelo SQL Editor do Supabase (ver seção 7 de docs/regras_padrao_petshop.md) — ainda não existe tela de onboarding."
      />
    );
  }
  ...
  return (
    <div className="space-y-3">
      {petshops.map((petshop) => (
        <PetshopTaxasCard key={petshop.id} petshop={petshop} />
      ))}
    </div>
  );
}

function PetshopTaxasCard({ petshop }: { petshop: Petshop }) {
  const [editando, setEditando] = useState(false);

  if (editando) {
    return <PetshopTaxasEditForm petshop={petshop} onCancel={() => setEditando(false)} onSaved={() => setEditando(false)} />;
  }
